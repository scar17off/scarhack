...
local DarkmodeToggle = VisualTab:CreateToggle({
    text = "Darkmode",
    default = false,
    callback = function(Value)
        -- Store the state for use in the slider
        DarkmodeToggle._enabled = Value
        -- Apply immediately if slider already set
        if DarkmodeToggle._darkness then
            local percent = DarkmodeToggle._darkness
                    ...
                    if model:IsA("Folder") or model:IsA("Model") then
                        for _, part in ipairs(model:GetDescendants()) do
                            if part:IsA("BasePart") then
                                if Value then
                                    part.Color = part.Color:lerp(Color3.new(0,0,0), percent)
                                else
                                    part.Color = part.Color:lerp(Color3.new(1,1,1), percent)
                                end
                            end
                        end
                    end
...
DarkmodeToggle:CreateSlider({
    ...
    callback = function(Value)
        ...
        DarkmodeToggle._darkness = percent
        -- Only apply if darkmode is enabled
        if DarkmodeToggle._enabled then
            local map = workspace:FindFirstChild("Map")
            if map then
                for _, model in ipairs(map:GetChildren()) do
                    if model:IsA("Folder") or model:IsA("Model") then
